...
function help(say, from, chan, message) {
  ...
  if (message.indexOf("!help") === 0) {
    ...
    (message.indexOf(NICK) !== -1 || privateMessage) &&
  ...
  if (!privateMessage) say(chan, from + ", see private messages.");
  ...
  if (topic) {
    ...
    say(
      from,
      "This is an instance of meta-bot (https://github.com/bnjbvr/meta-bot), a generic IRC bot that can be extended with modules to perform basic tasks."
    );
    ...
    say(
      from,
      "You can ask for more help about a specific module with !help MODULE_NAME."
    );
  }
